...
const BACKEND_URL = "https://api.haritindia.in/api";
...
describe("Buyer Registration", () => {
  ...
  test("Registration fails if OTP is invalid", async () => {
    const response = await axios.post(`${BACKEND_URL}/register`, {
      category: "buyer",
      mobNum: "9876543210",
      email: "test@example.com",
  ...
  test("Registration fails if passwords do not match", async () => {
    ...
    expect(response.status).toBe(400);
    expect(response.data.message).toBe("Passwords do not match");
  });

  test("Registration succeeds with valid input", async () => {
    // Simulate OTP saving
    await axios.post(`${BACKEND_URL}/save-otp`, {
      phone: "9876543210",
      otp: "1234",
    });
  ...
  test("Registration fails if token generation fails", async () => {
    jest
      .spyOn(tokenService, "generateAccessRefreshTokens")
      .mockImplementation(() => {
    ...
    expect(response.data.message).toBe("Failed to generate tokens!");
  });
});

describe("Buyer Login", () => {
  test("Login fails if mobile number is missing", async () => {
    const response = await axios.post(`${BACKEND_URL}/login`, {
      password: "123456",
    });

    expect(response.status).toBe(400);
    expect(response.data.message).toBe("Mobile number is required");
  });
  ...
  test("Login fails if token generation fails", async () => {
    const mobNum = "9876543210";
    const password = "123456";
...
describe("Feedback API", () => {
  test("Fails if rating is out of range", async () => {
    const response = await axios.post(
      ...
      {
        headers: { Authorization: `Bearer ${token}` }, // Assuming token is already available
      }
    );
